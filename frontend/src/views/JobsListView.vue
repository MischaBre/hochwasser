    ...
    <Card>
      ...
      <CardContent>
        <div class="mb-4 flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <Label for="include-disabled">{{ t('jobsList.includeDisabled') }}</Label>
        </div>
        ...
        <div v-if="jobsQuery.isLoading.value" class="text-sm text-muted-foreground">{{ t('jobsList.loading') }}</div>

        <div v-else-if="jobs.length === 0" class="rounded-md border bg-muted/30 p-4 text-sm text-muted-foreground">{{ t('jobsList.empty') }}</div>
          ...
          <table class="w-full min-w-[760px] border-collapse text-left text-sm">
            <thead>
                <tr class="border-b text-xs uppercase tracking-wide text-muted-foreground">
                  ...
                  <th class="py-3 text-right">{{ t('jobsList.table.actions') }}</th>
                </tr>
              </thead>
            <tbody>
              <tr v-for="job in jobs" :key="job.job_uuid" class="border-b last:border-none">
                ...
                <td class="py-3 pr-4 text-muted-foreground">{{ formatNumber(job.limit_cm) }} cm</td>
                <td class="py-3 pr-4 text-muted-foreground">{{ job.locale }}</td>
                <td class="py-3 pr-4">
                  <Badge :variant="job.enabled ? 'default' : 'secondary'">{{ job.enabled ? t('jobsList.enabled') : t('jobsList.disabled') }}</Badge>
                </td>
                <td class="py-3 text-right">
                  <div class="flex justify-end gap-2">
                    <Button variant="ghost" size="sm" @click="router.push({ name: 'job-details', params: { jobUuid: job.job_uuid } })">{{ t('jobsList.view') }}</Button>
                    <Button variant="ghost" size="sm" @click="router.push({ name: 'job-edit', params: { jobUuid: job.job_uuid } })">{{ t('jobsList.edit') }}</Button>
                    <Button
                      variant="destructive"
                      size="sm"
                    ...
                    >
                      {{ deleteMutation.isPending.value ? t('jobsList.deleting') : t('jobsList.delete') }}
                    </Button>
                  </div>
                </td>
              </tr>
